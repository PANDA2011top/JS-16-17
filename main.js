...
function addTodo() {
    ...
    if (todoText.length === 0) {
        alert("Iltimos, biror narsa kiriting ");
        return;
    }
    if (todoText.length > 38) {
        alert("Iltimos, kamroq matn kiriting!");
        return;
    }
    const todoObject = {
        text: todoText,
...
themeToggle.addEventListener('click', () => {
    if (body.getAttribute('data-theme') === 'dark') {
        ...
        themeToggle.textContent = 'Kungi rejim';
        localStorage.setItem('theme', 'light'); 
    } else {
        body.setAttribute('data-theme', 'dark');
        themeToggle.textContent = 'Tungi Rejim';
